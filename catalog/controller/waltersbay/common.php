...
// set title of the page
        $this->document->setTitle("Knowledge Base Page");
...
// set data to the variable
        $data['my_custom_text'] = "This is the knowledge base page.";

// call the "View" to render the output
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/waltersbay/knowledge_base.tpl')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/waltersbay/knowledge_base.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/waltersbay/knowledge_base.tpl', $data));
        }
    }
